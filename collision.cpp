...
float pointdistacesquarex(vector3d p1,vector3d p2)
{
	vector3d vec(p2.x-p1.x,p2.y-p1.y,p2.z-p1.z);
...
float rayplanedistx(float nx,float ny,float nz,float x0,float y0,float z0,float xs,float ys,float zs,float xd,float yd,float zd)
{
...
bool collision::sphereplane(vector3d& sp,vector3d pn,vector3d p1,vector3d p2,vector3d p3,vector3d p4, float r)
{
    ...
    if(rayplanedistx(pn.x,pn.y,pn.z,p1.x,p1.y,p1.z,sp.x,sp.y,sp.z,-pn.x,-pn.y,-pn.z)>r || rayplanedistx(-pn.x,-pn.y,-pn.z,p1.x,p1.y,p1.z,sp.x,sp.y,sp.z,pn.x,pn.y,pn.z)>r)
        return false;
	...
	{
		float raycol=rayplanedistx(pn.x,pn.y,pn.z,p1.x,p1.y,p1.z,sp.x,sp.y,sp.z,-pn.x,-pn.y,-pn.z);	//we calculate how far we are
		...
		{
			raycol=rayplanedistx(-pn.x,-pn.y,-pn.z,p1.x,p1.y,p1.z,sp.x,sp.y,sp.z,pn.x,pn.y,pn.z);
...
//{
//	return (pointdistacesquarex(c1,c2)<=(r1+r2)*(r1+r2));
//}

bool collision::spheresphere(vector3d& c1,float r1,vector3d c2,float r2)
{
    float dist=collision::pointdistace(c1,c2);
    if(dist<=(r1+r2)*(r1+r2))
    {
